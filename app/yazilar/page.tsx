...
import { yazilarListQuery, haberlerQuery, yazarlarQuery } from "@/lib/queries";
import YazilarClient from "./YazilarClient";
import type { Yazi, Haber, Yazar } from "./YazilarClient";

const playfair = Playfair_Display({ subsets: ["latin"] });

export default async function YazilarPage() {
  const [yazilar, haberler, yazarlar] = await Promise.all([
  ...
  return (
    <main className={`${playfair.className} min-h-screen bg-white`}>
      <YazilarClient yazilar={yazilar} haberler={haberler} yazarlar={yazarlar} />
    </main>
  );
}
